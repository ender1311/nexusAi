...
import { SLIDEUP_CATEGORY_VALUES, SLIDEUP_SUBCATEGORIES } from "@/lib/slideup-categories";
...
export async function DELETE(req: NextRequest) {
  ...
  try {
    ...
    return handleRouteError("DELETE /api/slideup-library", err);
  }
}
...
export async function GET(req: NextRequest) {
  try {
    ...
    const where: Prisma.MessageVariantWhereInput = {
      message: { agentId: null, channel: "in-app" },
    ...
    return handleRouteError("GET /api/slideup-library", err);
  }
}
...
export async function POST(req: NextRequest) {
  ...
  if (typeof category !== "string" || !SLIDEUP_CATEGORY_VALUES.includes(category)) return fail("invalid category", 400);
  ...
  if (subSlug && !SLIDEUP_SUBCATEGORIES[category]?.includes(subSlug)) return fail("invalid subcategory", 400);
