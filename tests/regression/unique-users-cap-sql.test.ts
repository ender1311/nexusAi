...
// This test exercises the exact SQL shape so a future query change breaks here,
...
describe("regression: uniqueUsersCap COUNT(DISTINCT) SQL shape", () => {
  it("returns n = 3 for three distinct users with decisions for the agent", async () => {
  ...
  it("counts distinct users — duplicate decisions for same user count once", async () => {
    const agent = await createAgent({ name: "Cap Agent Dedup" });

    // u1 has two decisions; should still count as 1 distinct user
    await prisma.userDecision.create({ data: { agentId: agent.id, userId: "u1", channel: "push" } });
    await prisma.userDecision.create({ data: { agentId: agent.id, userId: "u1", channel: "email" } });
    await prisma.userDecision.create({ data: { agentId: agent.id, userId: "u2", channel: "push" } });
  ...
  it("returns n = 0 when no decisions exist for the agent", async () => {
