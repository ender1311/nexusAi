...
describe("computeScheduledAt", () => {
  ...
  it("falls back to fallback hour when preferred time has already passed", () => {
    ...
    // fallbackSendHour = 8 → return today at 08:00 UTC with in_local_time=true
    // Braze re-interprets this hour in each user's local timezone and auto-rolls to tomorrow
    // for users whose local 8am has already passed — server-side date advancement is wrong.
    const result = computeScheduledAt(12, 0, 8, NOW);
    expect(result.scheduledAt).toEqual(new Date("2026-05-07T08:00:00Z"));
  ...
  it("returns today at fallback hour even when that UTC time has already passed", () => {
    // fallbackSendHour = 10 UTC, now is 14:00 UTC → 10:00 UTC has passed
    // We still return today at 10:00 UTC with in_local_time=true.
    // Braze handles per-user timezone rollover — US users (CDT=UTC-5) haven't hit 10am local yet.
    // Advancing server-side would push ALL users to tomorrow, breaking US timezones.
    const result = computeScheduledAt(null, null, 10, NOW);
    expect(result.scheduledAt).toEqual(new Date("2026-05-07T10:00:00Z"));
    expect(result.inLocalTime).toBe(true);
  });
  ...
  it("handles preferred hour at midnight (0) with subtraction crossing into negative minutes", () => {
    ...
    // Fallback: today at 08:00 UTC with in_local_time=true (Braze handles per-user rollover)
