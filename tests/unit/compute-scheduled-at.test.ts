...
describe("computeScheduledAt", () => {
  ...
  it("falls back to fallback hour when preferred time has already passed", () => {
    ...
    // fallbackSendHour = 8 → 08:00 UTC today has also passed → advance to tomorrow
  ...
  it("uses null preferred hour → falls back; returns tomorrow when fallback hour already passed", () => {
    // No preferred hour, fallbackSendHour = 20 UTC → 20:00 today is future (now is 14:00) → no advance
    const result = computeScheduledAt(null, null, 20, NOW);
    expect(result.scheduledAt).toEqual(new Date("2026-05-07T20:00:00Z"));
    expect(result.inLocalTime).toBe(true);

    // fallbackSendHour = 10 UTC, now is 14:00 UTC → 10:00 today has passed → advance to tomorrow
    const result2 = computeScheduledAt(null, null, 10, NOW);
    expect(result2.scheduledAt).toEqual(new Date("2026-05-08T10:00:00Z"));
    expect(result2.inLocalTime).toBe(true);
  });
  ...
  it("handles preferred hour at midnight (0) with subtraction crossing into negative minutes", () => {
    ...
    // Fallback 08:00 UTC today has also passed → advance to tomorrow
